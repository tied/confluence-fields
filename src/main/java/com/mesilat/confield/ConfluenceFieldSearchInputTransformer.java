...
public class ConfluenceFieldSearchInputTransformer extends MultiSelectCustomFieldSearchInputTransformer implements SearchInputTransformer {
    ...
    public void validateParams(
        ...
        final String title = (String)custParams.getFirstValueForNullKey();
        if ("-1".equals(title)){
            return;
        }
        try {
            DataServiceResult result = dataService.getPage(getCustomField().getIdAsLong(), title);
            ...
            errors.addError(getCustomField().getId(), i18n.getText("com.mesilat.confluence-field.err.pageNotFound", title));
        } catch(DataServiceException | IOException ex) {
            errors.addError(getCustomField().getId(), ex.getMessage());
        }
    }
